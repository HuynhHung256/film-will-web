  ...
  <div class="flex mx-36 my-10">
    <div>
      <div class="border-b-2 w-[90%]">
        ...
        <p class="text-xl">Choose the cinema schedule that you will watch</p>
        <div class="flex my-5 flex-wrap">
          ...
          <tr v-for="item in DateDetail" :key="item.daymonth">
            <button
              class="border rounded-lg p-4 mx-3 my-3 text-center cursor-pointer w-[85px] shadow hover:-translate-y-1 hover:scale-125 duration-500"
      ...
      <div>
        ...
        <div class="mt-5">
          ...
          <div class="flex my-4 space-x-6">
            ...
            >
              ...
              />
              <img src="@/page html/html nguyen/component/search.png" alt="#" />
            </div>
        ...
        <div class="w-[60%] mt-5">
          <!-- <TicketBook
            ...
            /> -->
          <div class="mt-5">
            <tr v-for="item in Class" :key="item.type">
      ...
      <div class="mt-5">
        <nuxt-link to="/films/14/choose-seat">
          ...
          <button type="button" class="bg-sky-800 hover:bg-sky-900 rounded-lg py-4 px-16 drop-shadow-md">
    ...
    <div class="w-[60%]">
      ...
      <div class="px-4 py-4 border-[3px] rounded-lg">
        <h1 class="text-[24px] font-semibold  text-sky-900 mb-2">
          {{ this.theater_name }}
        </h1>
        <i class="text-[18px] font-medium font-mono text-primary mb-4">
          {{ this.dateSelected.value }}
        </i>
        <div class="flex text-[20px] font-medium justify-between mb-3">
          <h1 class="font-semibold font-mono">Type:</h1>
          <i class="font-mono">{{ this.TypeSelected.type }}</i>
        </div>
        <div class="flex text-[20px] font-medium justify-between mb-3">
          <h1 class="font-semibold font-mono">Time:</h1>
          <i class="font-mono">{{ this.TypeSelected.time }}</i>
        </div>
        <div class="flex text-[20px] font-medium justify-between mb-3">
          <h1 class="font-semibold font-mono">Seats:</h1>
          <span>
            <i v-for="(seat, index) in this.Ticket.seat" :key="index" class="font-mono"
              >{{ seatWithComma(index) }}
            </i>
          </span>
        </div>
        <i
          ...
          class="text-red-500"
        >
          *Please complete your booking before payment
        </i>
        <nuxt-link to="/films/14/payment">
          <button
            class="text-white rounded-lg text-[20px] w-full py-4 font-bold mt-2 drop-shadow-md"
            :class="
              TypeSelected.time &&
              dateSelected.value &&
              Ticket.seat.length !== 0
...
export default {
      ...
      Class: [
        {
          type: "REGULAR 2D",
          price: 50000,
          time: ["11:00", "14:40", "15:45", "17:15", "18:15", "20:00", "21:00"],
        },
        {
          type: "GOLD CLASS 2D",
          price: 100000,
  ...
  methods: {
    ...
    setTicket(value) {
      ...
      this.TypeSelected = {
        type: value.target.parentElement.previousElementSibling.firstChild
          .innerText,
        time: value.target.innerText,
        price:
          value.target.parentElement.previousElementSibling.lastChild.innerText,
      }
    ...
    seatWithComma(index) {
      if (index !== this.Ticket.seat.length - 1) {
        return `${this.Ticket.seat[index]},`
      } else {
        return this.Ticket.seat[index]
      }
    },
